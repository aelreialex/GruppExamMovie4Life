  ...
  return (
      <section className="singleMovie">
          <div className="singleMovie__img-box">
            <img className="singleMovie__img" alt="test.Title" src={test.Poster}></img>
          </div>
          <article className="singleMovie__information">
            <h1 className="singleMovie__title">{test.Title}</h1>
            <p className="singleMovie__releaseDate"> Year: {test.Year}, Long: {test.Runtime}, Imdbrating:{test.imdbRating}</p>
            ...
            ? <button onClick={ () => removeFromWatchlist(movie.imdbID)} className="button button-single">Remove from Watchlist</button>
            : <button onClick={ () => addToWatchlist(movie)} className="button button-single">Add to Watchlist</button>
            }
            <h3 className="singleMovie__actors">Actors <br></br> <span className="singleMovie__actors-text">{test.Actors}</span></h3>
