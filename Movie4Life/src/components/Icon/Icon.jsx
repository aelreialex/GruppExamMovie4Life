...
const Icon = ({ path, type, onClick = null, faved = null }) => {
    ...
    return (
        <Link to={ path } className={`icon ${faved}`} onClick={ onClick } >
             <svg className={type === 'Home' ? homeClassName : type ==='Watchlist' ? watchClassName : ''} 
                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" >
                <path d={type === 'Home' ? homePath : type === 'Watchlist' ? watchPath : ''}/>
            </svg>
        </Link>
    )
}
